// import React from "react";
// import { loadStripe } from "@stripe/stripe-js";
// import axios from "axios";
// import { useLocation, useNavigate } from "react-router-dom";
// import { PayPalScriptProvider, PayPalButtons } from "@paypal/react-paypal-js";
// import NavbarComponent from "../components/NavbarComponent";

// const stripePromise = loadStripe(
//   "pk_test_51SWmLsGX7IdPsrl8yQT9MrxqPvECwkLnosfTur9r0gDRevhliGo0dbGWWUt9FP8MmE2IUKgmqO4ujwjVDF4tZQAs00ASxmd8gZ"
// );

// export default function PaymentPage() {
//   const location = useLocation();
//   const navigate = useNavigate();

//   // Handling Undefined State Safely
//   const { state } = location || {};

//   // Accepting Both "CartItems" And "Items" Keys To Ensure Compatibility
//   const cartItems = state?.items || state?.cartItems || [];
//   const total =
//     state?.total ||
//     (cartItems.length
//       ? cartItems.reduce((sum, item) => sum + item.price * item.quantity, 0)
//       : 0);

//   const orderId = state?.orderId || null;
//   const orderNumber = state?.orderNumber || state?.order?.order_number || null;

//   // Handling missing data (Direct Navigation / Refreshing)
//   if (!state || !cartItems.length) {
//     return (
//       <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 text-center">
//         <NavbarComponent />
//         <h1 className="text-2xl font-semibold text-gray-700 mb-4">
//           No payment data found
//         </h1>
//         <p className="text-gray-500 mb-6">
//           It seems you navigated here directly. Please go back to your cart.
//         </p>
//         <button
//           onClick={() => navigate("/cart")}
//           className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition"
//         >
//           Go Back to Cart
//         </button>
//       </div>
//     );
//   }

//   // Stripe Payment Handler
//   const handleStripeCheckout = async () => {
//     try {
//       const response = await axios.post(
//         "http://127.0.0.1:8000/api/create-checkout-session",
//         { products: cartItems }
//       );
//       window.location.href = response.data.url;
//     } catch (error) {
//       console.error("Stripe Error:", error);
//       alert("Payment failed. Try again.");
//     }
//   };

//   // PayPal Payment Handler
//   const handlePayPalApprove = async (orderID) => {
//     try {
//       const response = await axios.post(
//         "http://127.0.0.1:8000/api/paypal/capture-order",
//         { orderID }
//       );
//       alert("Payment successful via PayPal!");
//       console.log(response.data);
//       navigate("/"); // Redirecting to homepage or success page
//     } catch (error) {
//       console.error("PayPal Error:", error);
//       alert("Payment capture failed.");
//     }
//   };

//   return (
//     <div className="min-h-screen bg-gray-100 p-6">
//       <NavbarComponent />
//       <h1 className="text-3xl font-bold mb-8 text-gray-800 text-center">
//         Proceed to Payment
//       </h1>

//       <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
//         {/* LEFT SECTION - ORDER SUMMARY */}
//         <div className="bg-white shadow-md rounded-lg p-6">
//           <h2 className="text-xl font-semibold mb-4 text-gray-700 text-center">
//             Order Summary
//           </h2>
//           {orderId && (
//             <p className="text-center text-gray-600 mb-3">
//               <strong>Order ID:</strong> {orderNumber}
//             </p>
//           )}
//           <hr className="mb-6" />
//           <table className="w-full border-collapse text-black text-sm">
//             <thead>
//               <tr className="border-b text-left">
//                 <th className="pb-2 w-2/5">Product</th>
//                 <th className="pb-2 w-1/5 text-center">Qty</th>
//                 <th className="pb-2 w-1/5 text-right">Unit Price</th>
//                 <th className="pb-2 w-1/5 text-right">Total</th>
//               </tr>
//             </thead>

//             <tbody>
//               {cartItems.map((item, index) => (
//                 <tr key={index} className="border-b">
//                   <td className="py-2 truncate max-w-0" title={item.name}>
//                     {item.name}
//                   </td>
//                   <td className="py-2 text-center">{item.quantity}</td>
//                   <td className="py-2 text-right">
//                     ${Number(item.price).toFixed(2)}
//                   </td>
//                   <td className="py-2 text-right">
//                     ${(Number(item.price) * item.quantity).toFixed(2)}
//                   </td>
//                 </tr>
//               ))}
//             </tbody>
//           </table>

//           <div className="mt-2 pt-2">
//             <div className="flex justify-between text-xl font-bold text-gray-800">
//               <span>Total:</span>
//               <span>${total.toFixed(2)}</span>
//             </div>
//           </div>
//         </div>

//         {/* RIGHT SECTION - PAYMENT OPTIONS */}
//         <div className="bg-white shadow-md rounded-lg p-6 flex flex-col">
//           <h2 className="text-xl text-center font-semibold mb-4 text-gray-700">
//             Choose Payment Option
//           </h2>
//           <hr className="mb-6" />

//           <div className="flex flex-col gap-6 items-center max-w-md mx-auto w-full">
//             <button
//               onClick={handleStripeCheckout}
//               className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-lg shadow w-full max-w-sm"
//             >
//               Pay with Debit or Credit Card
//             </button>

//             <div className="w-full max-w-sm">
//               <PayPalScriptProvider
//                 options={{
//                   "client-id":
//                     "AXs--UziEnKcq_NwUDWcbTF8E5MB6GKqT4T5IP6bNEQ_VFNzA7jwrKGSHgY4Q_rodmpMGaA_QVFIpzqg",
//                   currency: "USD",
//                 }}
//               >
//                 <PayPalButtons
//                   style={{ layout: "vertical" }}
//                   fundingSource="paypal"
//                   createOrder={async () => {
//                     const response = await axios.post(
//                       "http://127.0.0.1:8000/api/paypal/create-order",
//                       { amount: total.toFixed(2) }
//                     );
//                     return response.data.id;
//                   }}
//                   onApprove={async (data) => {
//                     await handlePayPalApprove(data.orderID);
//                   }}
//                   onError={(err) => console.error("PayPal Error:", err)}
//                 />
//               </PayPalScriptProvider>
//             </div>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// }








import React from "react";
import { loadStripe } from "@stripe/stripe-js";
import axios from "axios";
import { useLocation, useNavigate } from "react-router-dom";
import { PayPalScriptProvider, PayPalButtons } from "@paypal/react-paypal-js";
import NavbarComponent from "../components/NavbarComponent";

const stripePromise = loadStripe(
  "pk_test_51SWmLsGX7IdPsrl8yQT9MrxqPvECwkLnosfTur9r0gDRevhliGo0dbGWWUt9FP8MmE2IUKgmqO4ujwjVDF4tZQAs00ASxmd8gZ"
);

export default function PaymentPage() {
  const location = useLocation();
  const navigate = useNavigate();

  const { state } = location || {};

  // ✅ Support both cart and order-based navigation
  const orderId = state?.orderId || null;
  const orderNumber = state?.orderNumber || state?.order?.order_number || null;
  const cartItems = state?.items || state?.cartItems || [];

  // ✅ Gracefully calculate total from either cartItems or direct total
  const total = Number(
    state?.total ||
      (cartItems.length
        ? cartItems.reduce((sum, item) => sum + item.price * item.quantity, 0)
        : 0)
  );

  // 🟡 Prevent blank screen — handle only if *absolutely no* data
  const hasOrderData = orderId || cartItems.length > 0 || total > 0;

  if (!hasOrderData) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 text-center">
        <NavbarComponent />
        <h1 className="text-2xl font-semibold text-gray-700 mb-4">
          No payment data found
        </h1>
        <p className="text-gray-500 mb-6">
          It seems you navigated here directly. Please go back to your cart or
          order history.
        </p>
        <button
          onClick={() => navigate("/cart")}
          className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition"
        >
          Go Back
        </button>
      </div>
    );
  }

  // ✅ Stripe Payment Handler
  const handleStripeCheckout = async () => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/create-checkout-session",
        { products: cartItems }
      );
      window.location.href = response.data.url;
    } catch (error) {
      console.error("Stripe Error:", error);
      alert("Payment failed. Try again.");
    }
  };

  // ✅ PayPal Payment Handler
  const handlePayPalApprove = async (orderID) => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/paypal/capture-order",
        { orderID }
      );
      alert("Payment successful via PayPal!");
      console.log(response.data);
      navigate("/"); // redirect on success
    } catch (error) {
      console.error("PayPal Error:", error);
      alert("Payment capture failed.");
    }
  };

  return (
    <div className="min-h-screen bg-gray-100 pt-24 px-4 sm:px-6 pb-6">
      <NavbarComponent />
      <h1 className="text-3xl font-bold mb-8 text-gray-800 text-center">
        Proceed to Payment
      </h1>

      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* LEFT SECTION - ORDER SUMMARY */}
        <div className="bg-white shadow-md rounded-lg p-6">
          <h2 className="text-xl font-semibold mb-4 text-gray-700 text-center">
            Order Summary
          </h2>

          {orderNumber && (
            <p className="text-center text-gray-600 mb-3">
              <strong>Order No:</strong> {orderNumber}
            </p>
          )}

          <hr className="mb-6" />

          {/* If we have items, display them, else show total only */}
          {cartItems.length > 0 ? (
            <div className="overflow-x-auto">
            <table className="w-full min-w-[480px] border-collapse text-black text-sm">
              <thead>
                <tr className="border-b text-left">
                  <th className="pb-2 w-2/5">Product</th>
                  <th className="pb-2 w-1/5 text-center">Qty</th>
                  <th className="pb-2 w-1/5 text-right">Unit Price</th>
                  <th className="pb-2 w-1/5 text-right">Total</th>
                </tr>
              </thead>
              <tbody>
                {cartItems.map((item, index) => (
                  <tr key={index} className="border-b">
                    <td className="py-2 truncate max-w-0" title={item.name}>
                      {item.name}
                    </td>
                    <td className="py-2 text-center">{item.quantity}</td>
                    <td className="py-2 text-right">
                      ${Number(item.price).toFixed(2)}
                    </td>
                    <td className="py-2 text-right">
                      ${(Number(item.price) * item.quantity).toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
            </div>
          ) : (
            <p className="text-center text-gray-500 mt-4">
              No product details available for this order.
            </p>
          )}

          <div className="mt-4 pt-2">
            <div className="flex justify-between text-xl font-bold text-gray-800">
              <span>Total:</span>
              <span>${total.toFixed(2)}</span>
            </div>
          </div>
        </div>

        {/* RIGHT SECTION - PAYMENT OPTIONS */}
        <div className="bg-white shadow-md rounded-lg p-6 flex flex-col">
          <h2 className="text-xl text-center font-semibold mb-4 text-gray-700">
            Choose Payment Option
          </h2>
          <hr className="mb-6" />

          <div className="flex flex-col gap-6 items-center max-w-md mx-auto w-full">
            <button
              onClick={handleStripeCheckout}
              className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-lg shadow w-full max-w-sm"
            >
              Pay with Debit or Credit Card
            </button>

            <div className="w-full max-w-sm">
              <PayPalScriptProvider
                options={{
                  "client-id":
                    "AXs--UziEnKcq_NwUDWcbTF8E5MB6GKqT4T5IP6bNEQ_VFNzA7jwrKGSHgY4Q_rodmpMGaA_QVFIpzqg",
                  currency: "USD",
                }}
              >
                <PayPalButtons
                  style={{ layout: "vertical" }}
                  fundingSource="paypal"
                  createOrder={async () => {
                    const response = await axios.post(
                      "http://127.0.0.1:8000/api/paypal/create-order",
                      { amount: total.toFixed(2) }
                    );
                    return response.data.id;
                  }}
                  onApprove={async (data) => {
                    await handlePayPalApprove(data.orderID);
                  }}
                  onError={(err) => console.error("PayPal Error:", err)}
                />
              </PayPalScriptProvider>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

